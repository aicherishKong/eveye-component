...
const App: React.FC = () => {
  ...
  // 处理日期范围变化
  const handleDateRangeChange = (dates: [Date | null, Date | null], isoStrings: [string | null, string | null]) => {
    setDateRange(dates);
  ...
  return (
    <div style={{ 
      ...
      <h1 style={{ color: '#fff', marginBottom: '24px' }}>DateRangePicker 暗黑主题演示</h1>
      ...
      {dateRange[0] && dateRange[1] && (
        <div style={{ 
          ...
          已选择日期范围: {dateRange[0].toLocaleDateString()} 至 {dateRange[1].toLocaleDateString()}
        </div>
      )}
    </div>
  );
